...
def _classify_span(span) -> str | None:
    ...
    # 3. Contextual Lead (ADP Preposition or npadvmod noun phrase like "The moment")
    ...
    # 5. Simultaneous Setup (SCONJ "as", "when", etc.)
    ...
    # 4. Symmetrical Compound (Echoing Hinge)
    ...
    # 1. Subject-First Opener (starts with subject, followed by verb)
    subj_tokens = [t for t in span if t.dep_ in ("nsubj", "nsubjpass")]
    if subj_tokens:
        subj = subj_tokens[0]
        # Ensure all content tokens before the subject are only determiners, possessives, adjectives, adverbs, or punctuation
        allowed_before = True
        for t in span:
            if t.i < subj.i:
...
def analyze_sentence_architecture(
    ...
    Targets 5 specific overused structures and marks them as '_stacked' if
    detected 3+ times within a 100-word moving window.
    """
    if not text.strip():
        if progress_callback is not None:
            progress_callback(100)
    ...
    # Process paragraph-by-paragraph to prevent cross-paragraph spans
    cursor = 0
    while cursor < len(text):
        match = re.search(r"\r?\n+", text[cursor:])
        if match is None:
